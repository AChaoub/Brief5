window.onload = ()=>{
    ...
    btn.onclick = () => {
        if( ExpReg1.test(Email.value) == true && Email.value == CEmail.value && Email.value!= ""){
            ...
            parag1.innerText = "Réesayer encore une fois la remplissage ";
            parag1.style.color = "red";
        }
        if(passw.value == Cpassw.value && passw.value  != "" ){
            parag2.innerText = "la vérification password été faite en succes ";
            parag2.style.color = "green";
        }
        else{
            parag2.innerText = "Réesayer encore une fois la remplissage ";
    ...
    passw.onkeyup = ()=>{
        passw.value = passw.value.toUpperCase();
    ...
    Select.onchange = () =>{
        var x = 2020 - parseInt(Select.value);
        if (x <21 ){
